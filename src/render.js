...
export function render({ searchQuery, sortAsc, editingId, blurredIds, startEdit, deleteEntry, toggleDone, onDayDelete }) {
  ...
  container.innerHTML = sortedGroups.map(([, day]) => {
    ...
    // Group entries within the day by hour bin
    const hourGroups = {};
    day.forEach(e => {
      const hk = hourKey(e.timestamp);
      if (!hourGroups[hk]) hourGroups[hk] = [];
      hourGroups[hk].push(e);
    });
    const sortedHours = Object.keys(hourGroups)
      .sort((a, b) => sortAsc ? a.localeCompare(b) : b.localeCompare(a));

    const rows = sortedHours.map(hk => {
      const hourEntries = hourGroups[hk];
      const entryRows = hourEntries.map(e => {
    ...
    return `<div class="day-group" data-day="${dayKey(day[0].timestamp)}">
      <div class="day-label">
        <svg class="day-chevron" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/></svg>
  ...
  // ── Day group collapse ─────────────────────────────────────────────────
  container.querySelectorAll('.day-label').forEach(label => {
    label.addEventListener('click', e => {
      if (e.target.closest('button')) return;
      label.closest('.day-group').classList.toggle('collapsed');
    });
  });
